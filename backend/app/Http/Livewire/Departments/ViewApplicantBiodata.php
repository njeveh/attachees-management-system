...
public $feedback;
    ...
    public function mount($id)
    {
        ...
        $this->attachee = $this->application->attachee;
        ...
        if(count($emergency_contacts)){
            $emergency_contacts->map(function($contact, $key){
                $this->emergency_contacts->push(['name' => $contact->name, 'relationship' =>$contact->relationship,
            'phone_number' => $contact->phone_number, 'id' => $contact->id]);
            
            });
        }else{
            $this->emergency_contacts->push(['name' => '', 'relationship' => '',
            'phone_number' => '', 'id' => null,]);
        }
        if(count($education_levels)){
            $education_levels->map(function($level, $key){
                $this->education_levels->push(['education_level' => $level->education_level, 'start_date' =>$level->start_date,
            'end_date' => $level->end_date, 'id' => $level->id]);
            });
        }else{
            $this->education_levels->push(['education_level' => '', 'start_date' =>'',
            'end_date' => '', 'id' => null,]);
        }
        if(count($skills)){
        ...
        if(count($referees)){
            $referees->map(function($referee, $key){
                $this->referees->push(['name' => $referee->name, 'relationship' =>$referee->relationship,
                'phone_number' => $referee->phone_number, 'email' => $referee->email, 'institution' =>$referee->institution,
                'position' => $referee->position_in_the_institution, 'id' => $referee->id]);
            });
        }else{
            $this->referees->push(['name' => '', 'relationship' => '',
                'phone_number' => '', 'email' => '', 'institution' => '',
                'position' => '', 'id' => null,],
                ['name' => '', 'relationship' => '', 'phone_number' => '',
                'email' => '', 'institution' => '', 'position' => '', 'id' => null,],
                );
        }
    }
