    ...
    <form class="mt-3" wire:submit.prevent="createAdvert">
        ...
        <div class="container">
            ...
            <div class="col form-group mb-3">
                ...
                @foreach ($study_areas as $study_area_option )
                    <option value='{{ $study_area_option->id }}'>{{ $study_area_option->title }}</option>
                @endforeach
                </select>
            </div>
                    ...
                    @foreach ($requirements as $key => $requirement)
                                ...
                                <div class="mb-3 form-group">
                                    <div class="input-group mb-3">
                                        <textarea class="form-control mb-3 @error('requirements.' . $key . '.requirement') is-invalid @enderror"
                                            id="input_{{ $key }}_requirement" wire:model.defer="requirements.{{ $key }}.requirement" rows="2"></textarea>
                                        <button type="button" wire:click="removeInput({{ $key }},'requirement')"
                        ...
                        @else
                            <li>
                                <div class="mb-3 form-group">
                                    <textarea class="form-control mb-3 @error('requirements.' . $key . '.requirement') is-invalid @enderror"
                                        id="input_{{ $key }}_requirement" wire:model.defer="requirements.{{ $key }}.requirement" rows="2"
                                        required></textarea>
                                    @error('requirements.' . $key . '.requirement')
                                        <span class="error text-danger">
                                            <strong>{{ $message }}</strong>
            ...
            <div class="form-group mb-3">
                <label for="quarter-1-vacancies" class="form-label">{{ __('Quarter 1 (July-Sept):') }}</label>
                <input type="number" wire:model="quarter1_vacancies"
                    class="form-control @error('quarter1_vacancies') is-invalid @enderror" id="quarter-1-vacancies"
                    required>
            ...
            <div class="form-group mb-3">
                <label for="quarter-2-vacancies" class="form-label">{{ __('Quarter 2 (Oct-Dec):') }}</label>
                <input type="number" wire:model="quarter2_vacancies"
                    class="form-control @error('quarter2_vacancies') is-invalid @enderror" id="quarter-2-vacancies"
                    required>
            ...
            <div class="form-group mb-3">
                <label for="quarter-3-vacancies" class="form-label">{{ __('Quarter 3 (Jan-Mar):') }}</label>
                <input type="number" wire:model="quarter3_vacancies"
                    class="form-control @error('quarter3_vacancies') is-invalid @enderror" id="quarter-3-vacancies"
                    required>
            ...
            <div class="form-group mb-3">
                <label for="quarter-4-vacancies" class="form-label">{{ __('Quarter 4 (Apr-Jun):') }}</label>
                <input type="number" wire:model="quarter4_vacancies"
                    class="form-control @error('quarter4_vacancies') is-invalid @enderror" id="quarter-4-vacancies"
                    required>
